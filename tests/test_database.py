...
def test_available_buns(database):
    ...
    # Получаем список булочек из базы данных
    buns = database.available_buns()

    # Проверяем, что возвращается список булочек
    assert isinstance(buns, list), "Ожидался список булочек"
    assert len(buns) == 3, "Ожидалось 3 булочки"

    # Проверяем, что каждый элемент списка является экземпляром класса Bun
...
def test_available_ingredients(database):
    ...
    # Получаем список ингредиентов из базы данных
    ingredients = database.available_ingredients()

    # Проверяем, что возвращается список ингредиентов
    assert isinstance(ingredients, list), "Ожидался список ингредиентов"
    assert len(ingredients) == 6, "Ожидалось 6 ингредиентов"

    # Проверяем, что каждый элемент списка является экземпляром класса Ingredient
